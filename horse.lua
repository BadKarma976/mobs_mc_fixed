...
local horse = {
	...
	passive = true,
	hp_min = 15,
	hp_max = 30, -- not changed or made by me
	floats = 1,
	lava_damage = 4,
	water_damage = 1,
	...
	do_custom = function(self, dtime)
		...
		if not self.v2 then
			...
			self.accel = 6
			self.terrain_type = 3
			self.driver_attach_at = {x = 0, y = 4.5, z = -1.5}
			self.driver_eye_offset = {x = 0, y = 10, z = -3}
			
			local vs = self.visual_size or {x = 3.0, y = 3.0}
			self.driver_scale = {x = 1 / vs.x, y = 1 / vs.y}
		end
		...
		if self._regentimer >= 4 then
			if self.health < self.object:get_properties().hp_max then
				-- changed self.hp_max to self.object:get_properties().hp_max because of local varible
